...
export const inject = ['timer', 'llm', 'harness']

export function apply(ctx) {
  var settings = ctx.get('settings')
  var cfg = ctx.get('config') || {}

  // Step 1: Register in Settings → Models FIRST (this registers the meeting-notes namespace)
  try {
  ...
  // Step 2: Read config: settings system → cordis.yml → env var → defaults
  ...
  var harness = ctx.harness || { handle: function() {} }
  ...
  harness.handle('meeting-notes:save-config', async function(args) {
    if (args && args.apiKey && typeof args.apiKey === 'string' && args.apiKey.trim()) {
      ...
      // Persist to DSH settings system (survives restart)
      if (settings && typeof settings.update === 'function') {
        try {
          await settings.update('meeting-notes', { apiKey: apiKey })
        } catch (e) {
          console.warn('[meeting-notes] Settings system not available, using in-memory only:', e.message)
        }
      }
