    ...
    exports.apply = function(ctx) {
      ...
      function SidebarEntry(props) {
        ...
        function handleClick() {
          // Try to switch to the '听记' conversation view tab
          try {
            var cv = ctx.get('conversationView')
            if (cv && typeof cv.setActiveView === 'function') {
              cv.setActiveView('听记')
            }
          } catch (e) {}
          // Also load notes
      ...
      // When the "听记" tab is active, show the meeting notes UI
      try {
        slots.register({ name: 'conversation.view', id: '听记', order: 60, label: '听记' }, function() {
