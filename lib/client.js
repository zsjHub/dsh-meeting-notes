window.__ModuleLoader__.load({
  ...
  factory: function(require) {
    ...
    exports.apply = function(ctx) {
        ...
        async function handleStart() {
          try {
            ...
            if (!asrStarted) {
              ...
              // In demo mode we still need to mark the session as started
              // but there's no real microphone input
            }
      ...
      slots.inject('settings.section', function() { return slots.register({ name:'settings.section', id:'meeting-notes', order:50, label:'听记' }, function(p) { return React.createElement(MeetingNotesSettings, { close:p.close }) }) })
