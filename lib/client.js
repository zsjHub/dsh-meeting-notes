window.__ModuleLoader__.load({
  ...
  factory: function(require) {
    ...
    exports.inject = ['timer', 'settingsScope']

    exports.apply = function(ctx) {
      var slots = ctx.get('slots')
      if (!slots) { console.warn('[meeting-notes] Slots not available'); return }

      // Use settingsScope (client-side API) if available, fall back to settings (host-side API)
      var settingsScope = ctx.settingsScope
      var settings = null
      var meetingNotesScope = null

      // Try to get the settings scope for the meeting-notes namespace
      if (settingsScope && typeof settingsScope.bind === 'function') {
        try {
          meetingNotesScope = settingsScope.bind({ namespace: 'meeting-notes' })
          console.log('[meeting-notes] Using settingsScope service')
        } catch (e) {
          console.warn('[meeting-notes] Failed to bind settingsScope:', e.message)
        }
      }

      // Fallback: try ctx.get('settings')
      if (!meetingNotesScope) {
        settings = ctx.get('settings')
        if (settings && typeof settings.get === 'function') {
          console.log('[meeting-notes] Using settings service (fallback)')
        }
      }
      ...
      function loadNotesFromSettings() {
        if (meetingNotesScope && typeof meetingNotesScope.getSnapshot === 'function') {
          try {
            var snapshot = meetingNotesScope.getSnapshot()
            var notes = snapshot && snapshot.value ? snapshot.value._notes : null
            if (notes && Array.isArray(notes)) {
              notesCache = notes
            }
          } catch (e) {}
        } else if (settings && typeof settings.get === 'function') {
          try {
      ...
      function saveTranscriptToHost(text) {
        // Primary: use settingsScope
        if (meetingNotesScope && typeof meetingNotesScope.set === 'function') {
          try {
            meetingNotesScope.set('_transcript', text)
            meetingNotesScope.set('_timestamp', Date.now())
          } catch (e) {}
        } else if (settings && typeof settings.update === 'function') {
          try {
      ...
      function triggerFinalSummary(fullText) {
        ...
        if (meetingNotesScope && typeof meetingNotesScope.set === 'function') {
          try {
            meetingNotesScope.set('_finalTranscript', fullText)
            meetingNotesScope.set('_timestamp', Date.now())
          } catch (e) {}
        } else if (settings && typeof settings.update === 'function') {
          try {
