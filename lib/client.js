    ...
    exports.apply = function(ctx) {
      ...
      function loadFromStorage() {
        try {
          ...
          if (stored) {
            ...
            // Reconstruct activeNoteData from notesList to ensure it's the same object reference
            activeNoteData = null
            if (activeNoteId) {
              for (var i = 0; i < notesList.length; i++) {
                if (notesList[i].id === activeNoteId) {
      ...
      function saveToStorage() {
        try {
          localStorage.setItem('dsh-meeting-notes', JSON.stringify({
            ...
            // Note: activeNoteData is intentionally NOT saved separately
            // to avoid deserialization creating a separate object reference.
            // It is always a reference to an object inside notesList.
          }))
        } catch (e) {
      ...
      function MeetingNotesApp() {
        ...
        function handleNewRecording() {
          if (isCreating) {
            console.warn('[meeting-notes] handleNewRecording already in progress, skipping')
            return
          }
          isCreating = true
          try {
            ...
            activeNoteData = note
            saveToStorage()
            console.log('[meeting-notes] Created new note:', note.id, 'list length:', notesList.length)
        ...
        function stopRecording() {
          ...
          if (activeNoteData) {
            ...
            activeNoteData.duration = timerSeconds
            activeNoteData.transcript = fullTranscript || lastInterimText || ''
            activeNoteData.endTime = nowISO()
            saveToStorage()
          }
          if (timerInterval) { clearInterval(timerInterval); timerInterval = null }
          // Update the detail view with the saved transcript
          if (updateFn) updateFn()
        }
        ...
        async function handleSummarize(id) {
          ...
          setNotes([].concat(notesList))
          if (!result.success) {
            console.warn('[meeting-notes] Summarize failed:', result.error)
            alert('生成摘要失败：' + result.error)
          }
        }
        ...
        function renderDetail() {
          ...
          return React.createElement('div', { style: { flex: 1, overflow: 'auto', padding: '20px' } },
            ...
            !hasSummary && note.transcript
              ...
              : null,
            // Live transcript (during recording or just finished)
            (recStatus === 'recording' || (recStatus === 'idle' && liveTranscript))
              ? React.createElement('div', { style: { marginBottom: '16px' } },
                  React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' } },
                    React.createElement('span', { style: { fontSize: '14px' } }, '🎤'),
                    React.createElement('span', { style: { fontSize: '13px', fontWeight: '600', color: C.text } }, '实时转写'),
                    recStatus === 'recording'
                      ? React.createElement('span', { style: { fontSize: '11px', color: C.danger, animation: 'pulse 1.5s infinite', marginLeft: '6px' } }, '● 录音中')
                      : null
                  ),
                  // Accumulated final transcript
                  liveTranscript
                    ? React.createElement('div', { style: { fontSize: '13px', lineHeight: '1.7', color: C.text, padding: '12px 14px', backgroundColor: C.card, borderRadius: '8px', border: '1px solid ' + C.border, whiteSpace: 'pre-wrap', marginBottom: interimText ? '4px' : '0' } },
                        liveTranscript
                      )
                    : React.createElement('div', { style: { fontSize: '13px', color: C.textSecondary, padding: '12px 14px', backgroundColor: C.card, borderRadius: '8px', border: '1px dashed ' + C.border, textAlign: 'center' } },
                        '等待语音输入...'
                      ),
                  // Interim text (partial, not yet final)
                  interimText
                    ? React.createElement('div', { style: { fontSize: '13px', lineHeight: '1.7', color: C.textSecondary, padding: '4px 14px', whiteSpace: 'pre-wrap', fontStyle: 'italic' } },
                        interimText
                      )
                    : null
                )
      ...
      // Use slots.inject which waits for the slot to be declared by the web package
      slots.inject('sidebar.footer.action', function() {
        return slots.register({ name: 'sidebar.footer.action', id: '听记', order: 10, label: '听记' }, function() {
      ...
      slots.inject('shell.overlay', function() {
        return slots.register({ name: 'shell.overlay', id: 'meeting-notes', order: 10, label: '听记' }, function() {
