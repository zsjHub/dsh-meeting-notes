  ...
  factory: function(require) {
    ...
    exports.apply = function(ctx) {
      ...
      function MeetingNotesApp() {
        ...
        var _g = React.useState(false), showSettings = _g[0], setShowSettings = _g[1]
        ...
        return React.createElement('div', { style:{ padding:'16px', fontFamily:'-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif' } },
          ...
          showSettings ? React.createElement('div', {
            ...
            onClick:function(e) { if (e.target === e.currentTarget) setShowSettings(false) }
          },
            React.createElement('div', { style:{ background:C.card, borderRadius:'12px', width:'480px', maxWidth:'90vw', maxHeight:'80vh', overflowY:'auto', padding:'24px' } },
              React.createElement('div', { style:{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' } },
                React.createElement('h3', { style:{ margin:0, fontSize:'18px', fontWeight:'600', color:C.text } }, '会议听记设置'),
                React.createElement('button', { onClick:function() { setShowSettings(false) }, style:{ border:'none', background:'none', fontSize:'24px', cursor:'pointer', color:C.textSecondary, padding:'0', lineHeight:'1' } }, '×')
              ),
              React.createElement('label', { style:{ display:'block', marginBottom:'8px', fontSize:'14px', color:C.textSecondary, fontWeight:'500' } }, 'DeepSeek API Key'),
                ...
                async function handleSave() {
                  ...
                  try {
                    if (settings && typeof settings.update === 'function') {
                      settings.update('meeting-notes', { _command: 'save-config', apiKey: inputKey.trim() })
                    }
                    setSavedMsg('✓ 已保存')
                    setTimeout(function() { setSavedMsg('') }, 3000)
                  } catch (e) { setSavedMsg('保存失败: ' + e.message) }
                }
                return React.createElement('div', null,
                  React.createElement('div', { style:{ display:'flex', gap:'8px' } },
                    React.createElement('input', {
                      ...
                      placeholder:'sk-...',
                      style:{ flex:1, padding:'10px 14px', border:'1px solid ' + C.border, borderRadius:'8px', fontSize:'14px', outline:'none', boxSizing:'border-box' }
                    }),
                    React.createElement('button', {
                      onClick:handleSave,
                      style:{ padding:'10px 24px', backgroundColor:C.primary, color:'#fff', border:'none', borderRadius:'8px', cursor:'pointer', fontSize:'14px', fontWeight:'500', whiteSpace:'nowrap' }
                    }, savedMsg || '保存')
                  ),
                  savedMsg ? React.createElement('div', { style:{ marginTop:'8px', fontSize:'13px', color:C.success } }, savedMsg) : null,
                  React.createElement('div', { style:{ marginTop:'20px', padding:'16px', backgroundColor:C.bg, borderRadius:'8px', fontSize:'13px', color:C.textSecondary, lineHeight:'1.7' } },
                    ...
                    React.createElement('div', { style:{ marginTop:'8px', color:C.danger, fontSize:'12px' } }, '注意：通过此页面保存的 API Key 仅在当前会话有效，重启后需重新设置。')
                  )
                )
              }),
            )
          ...
          React.createElement('div', { style:{ backgroundColor:C.card, borderRadius:'12px', boxShadow:'0 1px 4px ' + C.shadow, overflow:'hidden', animation:'fadeIn 0.3s ease' } },
            ...
            React.createElement('div', { style:{ padding:'14px 20px', display:'flex', gap:'8px', alignItems:'center', borderBottom:'1px solid ' + C.border } },
              ...
              React.createElement('button', {
                ...
                style:Object.assign({}, btnStyle, { backgroundColor:'transparent', color:C.textSecondary, border:'1px solid ' + C.border, marginLeft:'auto', padding:'10px 16px', fontSize:'13px' })
              }, '⚙ 设置')
            ),
